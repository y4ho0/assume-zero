...
- Raw secrets passed as command arguments remain visible to the operating system while the process runs. A single opaque shell script is hidden from command metadata, but transformed/echoed values and undeclared short-option meanings cannot be redacted reliably.
