...
Verbose output is emitted only after bounded capture and redaction, rather than streamed before the redaction boundary. Raw CLI values still exist in process memory and the operating system's process argument view while the command runs. A single opaque `--shell` script is hidden in command displays and persisted command metadata because its option semantics cannot be parsed reliably; values echoed by that script may still evade output redaction. Do not place real credentials directly in command arguments.
